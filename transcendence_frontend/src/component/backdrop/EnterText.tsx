...
const EnterText:React.FC<EnterTextProps> = ({socket, jwtToken}) =>{
  ...
  const handleClick = () => {
    if (socket) {
      const message = { text: messageText, token: jwtToken }; // add user to message
      socket.emit('message', message); // emit the message to the server via the socket
    }
